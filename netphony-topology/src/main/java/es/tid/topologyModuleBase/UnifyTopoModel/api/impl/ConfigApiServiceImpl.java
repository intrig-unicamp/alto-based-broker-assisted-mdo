...
//		return Response.ok().entity(tSchema).build();
    ...
    public Response topologyFromTADS(SecurityContext securityContext)
        ...
        tSchema.version("001");
        tSchema.setName("Topology from TADS example");
    ...
    public Response topologyFromALTO(SecurityContext securityContext)
        ...
        try {
            ...
            TopologyModuleParams params = TopologyServerUnify
                    .getTopologyParams();
            ALTO_SERVER_IP = params.getAltoServer();
            ALTO_SERVER_PORT = params.getAltoServerPort();
            ALTO_URL_NETWORK_MAP = params.getAltoNetworkMapURL();
            ALTO_NETWORK_MAP_NAME = params.getAltoNetworkMapName();
            ALTO_NETWORK_MAP_CONTENT_TYPE = params
                    .getAltoNetworkMapContentType();

            JSONObject jsonObj = GET_REST_Query(getALTOURL(ALTO_SERVER_IP,
                    ALTO_SERVER_PORT, ALTO_URL_NETWORK_MAP,
                    ALTO_NETWORK_MAP_NAME));
            ...
            if (jsonObj.get(key) instanceof JSONObject) {
                ...
                while (lstkey.hasNext()) {
                    ...
                    /*
                    if (jSon.get(key) instanceof JSONObject) {
                        JSONObject jSonAux = new JSONObject(jSon.get(key)
                                .toString());
                        Iterator<?> lstkeyF = jSonAux.keys();
                        while (lstkeyF.hasNext()) {
                            key = (String) lstkeyF.next();

                            System.out.println(key + "-" + jSonAux.get(key));
                        }
                    }*/
                }
            }
