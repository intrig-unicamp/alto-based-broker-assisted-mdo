		...
		try
		{
            ...
            for (int i = 0; i < list_nodes_Unify.getLength(); i++) {
                ...
                if (nodes_Unify.getElementsByTagName("import_ip").getLength() > 0) {
                    ...
                    littleParams
                            .setRemoteUnifyhost((getCharacterDataFromElement(((Element) nodes_Unify
                                    .getElementsByTagName("import_ip").item(0)))));
                    littleParams
                            .setRemoteUnifyPort(Integer
                                    .parseInt((getCharacterDataFromElement(((Element) nodes_Unify
                                            .getElementsByTagName("import_port")
                                            .item(0))))));
                    littleParams.setUnifyReading(true);
                }
                if (nodes_Unify.getElementsByTagName("serve_port").getLength() > 0) {
                    littleParams
                            .setExportUnifyPort(Integer
                                    .parseInt((getCharacterDataFromElement(((Element) nodes_Unify
                                            .getElementsByTagName("serve_port")
                                            .item(0))))));
                    littleParams.setUnifyWritting(true);
                }                
                
                NodeList list_nodes_ALTO = doc.getElementsByTagName("alto");

                for (i = 0; i < list_nodes_ALTO.getLength(); i++) {
                    Element nodes_ALTO = (Element) list_nodes_ALTO.item(i);
                    //TopologyModuleParams littleParams = new TopologyModuleParams();
                    if (nodes_ALTO.getElementsByTagName("server").getLength() > 0) {
                        // nodes_ALTO = (Element) ((Element)
                        // nodes_ALTO.getElementsByTagName("server").item(0)).getElementsByTagName("").item(0);
                        littleParams
                                .setAltoServer((getCharacterDataFromElement((Element) ((Element) nodes_ALTO
                                        .getElementsByTagName("server").item(0))
                                        .getElementsByTagName("serverIP").item(0))));
                        log.info("ALTO Server: " + littleParams.getAltoServer());
                        littleParams
                                .setAltoServerPort((getCharacterDataFromElement((Element) ((Element) nodes_ALTO
                                        .getElementsByTagName("server").item(0))
                                        .getElementsByTagName("serverPort").item(0))));
                        log.info("ALTO Server Port: "
                                + littleParams.getAltoServerPort());
                    }
                    /*
                    if (nodes_ALTO.getElementsByTagName("neo4j").getLength() > 0) {

                        littleParams
                                .setAltoNeo4j((getCharacterDataFromElement((Element) ((Element) nodes_ALTO
                                        .getElementsByTagName("neo4j").item(0))
                                        .getElementsByTagName("serverIP").item(0))));
                        log.info("Neo4j Server: " + littleParams.getAltoNeo4j());
                        littleParams
                                .setAltoNeo4jPort((getCharacterDataFromElement((Element) ((Element) nodes_ALTO
                                        .getElementsByTagName("neo4j").item(0))
                                        .getElementsByTagName("serverPort").item(0))));
                        log.info("Neo4j Server Port: "
                                + littleParams.getAltoNeo4jPort());
                    }*/
                    if (nodes_ALTO.getElementsByTagName("networkMap").getLength() > 0) {

                        littleParams
                                .setAltoNetworkMapURL((getCharacterDataFromElement((Element) ((Element) nodes_ALTO
                                        .getElementsByTagName("networkMap").item(0))
                                        .getElementsByTagName("url").item(0))));
                        log.info("Network Map URL: "
                                + littleParams.getAltoNetworkMapURL());
                        littleParams
                                .setAltoNetworkMapContentType((getCharacterDataFromElement((Element) ((Element) nodes_ALTO
                                        .getElementsByTagName("networkMap").item(0))
                                        .getElementsByTagName("contentType").item(0))));
                        log.info("Network Map Content Type: "
                                + littleParams.getAltoNetworkMapContentType());
                        littleParams
                                .setAltoNetworkMapName((getCharacterDataFromElement((Element) ((Element) nodes_ALTO
                                        .getElementsByTagName("networkMap").item(0))
                                        .getElementsByTagName("name").item(0))));
                        log.info("Network Map Name: "
                                + littleParams.getAltoNetworkMapName());
                    }                    
                }                
