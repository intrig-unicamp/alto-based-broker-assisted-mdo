...
import edu.unicamp.intrig.p5gex.interDomainBase.UnifyTopoModel.model.Links;
...
public class FileTEDBUpdater {
     ...
     */
    public static SimpleDirectedWeightedGraph<Object, IntraDomainEdge> readNetwork(
            String fileName) {
        return FileTEDBUpdater.readNetwork(fileName, null, false, 0,
                Integer.MAX_VALUE);
    }
     ...
     */
    public static SimpleDirectedWeightedGraph<Object, IntraDomainEdge> readNetwork(
            String fileName, String layer) {
        return FileTEDBUpdater.readNetwork(fileName, layer, false, 0,
                Integer.MAX_VALUE, false);
    }

    /**
     * Reads a specific layer from a Network XML file It can treat all domains
     * as a single domain
     * 
     * @param fileName
     *            Name of the XML file
     ...
     *            Layer
     * @param allDomains
     *            if all domains are read or not
     * @return Graph of the network
     */
    public static SimpleDirectedWeightedGraph<Object, IntraDomainEdge> readNetwork(
            String fileName, String layer, boolean allDomains) {
        return FileTEDBUpdater.readNetwork(fileName, layer, allDomains, 0,
                Integer.MAX_VALUE, false);
    }
    ...
    public static SimpleDirectedWeightedGraph<Object, IntraDomainEdge> readNetwork(
            String fileName, String layer, boolean allDomains, int lambdaIni,
            int lambdaEnd) {
        return FileTEDBUpdater.readNetwork(fileName, layer, allDomains, 0,
                Integer.MAX_VALUE, false);
    }
     ...
     *            last lamnda (n)
     * @param isSSONnetwork
     *            If the network supports Flexi Grid
     * @return Graph of the network.
     */
    public static SimpleDirectedWeightedGraph<Object, IntraDomainEdge> readNetwork(
            String fileName, String layer, boolean allDomains, int lambdaIni,
            int lambdaEnd, boolean isSSONnetwork) {
        ...
        SimpleDirectedWeightedGraph<Object, IntraDomainEdge> graph = new SimpleDirectedWeightedGraph<Object, IntraDomainEdge>(
                IntraDomainEdge.class);
        ...
        try {
                ...
                if (readNetwork) {
                    ...
                    for (int i = 0; i < nodes.getLength(); i++) {
                        ...
                        if (SID_e != null) {
                            log.debug("SID exist");
                            int SID = Integer
                                    .parseInt(getCharacterDataFromElement(SID_e));
            ...
            for (int j = 0; j < nodes_domains.getLength(); j++) {
                ...
                if (readNetwork) {
                    ...
                    NodeList edgeCommon = doc
                            .getElementsByTagName("edgeCommon");
                    int grid = 0;
                    int cs = 0;
                    int n = 0;
                    for (int i = 0; i < edgeCommon.getLength(); i++) {

                        Element edgeCommonElement = (Element) edgeCommon
                                .item(i);
                        NodeList availableLabels_node = edgeCommonElement
                                .getElementsByTagName("AvailableLabels");
                        for (int k = 0; k < availableLabels_node.getLength(); k++) {

                            Element availableLabels_e = (Element) availableLabels_node
                                    .item(k);
                            NodeList labelSetField_node = availableLabels_e
                                    .getElementsByTagName("LabelSetField");
                            Element labelSetField_el = (Element) labelSetField_node
                                    .item(0);
                            if (labelSetField_el.getAttributeNode("type")
                                    .getValue().equals("4")) {// Tengo BitMapSet
                                // Crear un BitMapSet

                                NodeList numbLabels_node = labelSetField_el
                                        .getElementsByTagName("numLabels");

                                Element numbLabels_e = (Element) numbLabels_node
                                        .item(0);
                                String numbLabels_s = getCharacterDataFromElement(numbLabels_e);
                                numLabels = Integer.parseInt(numbLabels_s);

                                NodeList baseLabel_node = labelSetField_el
                                        .getElementsByTagName("baseLabel");
                                Element baseLabel_e = (Element) baseLabel_node
                                        .item(0);

                                float min_frequency;

                                grid = Integer.parseInt(baseLabel_e
                                        .getAttributeNode("grid").getValue());

                                cs = Integer.parseInt(baseLabel_e
                                        .getAttributeNode("cs").getValue());
                                boolean n_frequency_included = false;
                                if (baseLabel_e.getAttributeNode("n") != null) {
                                    n = Integer.parseInt(baseLabel_e
                                            .getAttributeNode("n").getValue());
                                    n_frequency_included = true;
                                } else if (baseLabel_e
                                        .getAttributeNode("min_frequency") != null) {
                                    String s_min_frequency = labelSetField_el
                                            .getAttributeNode("min_frequency")
                                            .getValue();

                                    min_frequency = Float
                                            .parseFloat(s_min_frequency);
                    ...
                    /*
                     * if(isSSONnetwork ==true){ if(cs==4){
                     * numLabels=numLabels*4; } else if (cs==5){
                     * numLabels=numLabels*8; } }
                     */

                    NodeList edges = element_domain
                    ...
                    for (int i = 0; i < edges.getLength(); i++) {
                        ...
                        if (type.equals("intradomain")) {
                            ...
                            Element source_if_id_el = (Element) source_if_id_nl
                                    .item(0);
                            ...
                            NodeList source_Numif_id_nl = source_router_el
                                    .getElementsByTagName("NumIf_id");
                            ...
                            if (source_Numif_id_el != null) {
                                ...
                                try { // src_Numif_id type : Inet4Address
                                    src_Numif_id = (Inet4Address) Inet4Address
                                            .getByName(s_source_Numif_id);
                                } catch (Exception e) { // src_Numif_id type :
                                                        // DataPathID
                                    src_Numif_id = DataPathID
                                            .getByName(s_source_Numif_id);
                                }
                            }
                            ...
                            // Anyadimos los SID
                            if (SIDS.get(s_router_id_addr) != null
                                    && SIDS.get(d_router_id_addr) != null) {
                                log.debug("setting SIDS src: "
                                        + SIDS.get(s_router_id_addr) + " dst: "
                                        + SIDS.get(d_router_id_addr));
                                edge.setSrc_sid(SIDS.get(s_router_id_addr));
                                edge.setDst_sid(SIDS.get(d_router_id_addr));
                                log.debug("edge.getSrc_sid(): "
                                        + edge.getSrc_sid());
                                log.debug("edge.getDst_sid(): "
                                        + edge.getDst_sid());
                            } else if (SIDSDP.get(s_router_id_addr) != null
                                    && SIDSDP.get(d_router_id_addr) != null) {
                                log.debug("setting SIDSDP src: "
                                        + SIDSDP.get(s_router_id_addr)
                                        + " dst: "
                                        + SIDSDP.get(d_router_id_addr));
                            ...
                            NodeList dest_if_id_nl = dest_el
                                    .getElementsByTagName("if_id");
                            Element dest_if_id_el = (Element) dest_if_id_nl
                                    .item(0);
                            String s_dest_if_id;
                            int dst_if_id = -1;
                            ...
                            NodeList dest_Numif_id_nl = dest_el
                                    .getElementsByTagName("NumIf_id");
                            Element dest_Numif_id_el = (Element) dest_Numif_id_nl
                                    .item(0);
                            ...
                            if (source_Numif_id_el != null) {
                                ...
                                try { // s_dest_Numif_id type : Inet4Address
                                    dst_Numif_id = (Inet4Address) Inet4Address
                                            .getByName(s_dest_Numif_id);
                                } catch (Exception e) { // s_dest_Numif_id type
                                                        // : DataPathID
                                    dst_Numif_id = DataPathID
                                            .getByName(s_dest_Numif_id);
                                }
                            }
                            ...
                            // DELAY, IF IT COMES..
                            NodeList delay_ms_nl = element
                                    .getElementsByTagName("delay");
                            if (delay_ms_nl.getLength() > 0) {

                                Element delay_ms_el = (Element) delay_ms_nl
                                        .item(0);
                                String s_delay_ms = getCharacterDataFromElement(delay_ms_el);
                                double delay_ms = Double
                                        .parseDouble(s_delay_ms);
                                edge.setDelay_ms(delay_ms);
                            }

                            // TE Link information
                            NodeList maximum_bandwidth_nl = element
                                    .getElementsByTagName("maximum_bandwidth");
                            if (maximum_bandwidth_nl != null) {
                                if (maximum_bandwidth_nl.getLength() > 0) {
                                    if (edge.getTE_info() == null) {
                                        ...
                                        TE_Information te_info = edge
                                                .getTE_info();
                                        if (commonBitmapLabelSet) {
                                            // if(lambdaEnd!=Integer.MAX_VALUE){

                                            // te_info.createBitmapLabelSet(numLabels,
                                            // grid, cs, n,lambdaIni,lambdaEnd);
                                            // }
                                            // else
                                            te_info.createBitmapLabelSet(
                                                    numLabels, grid, cs, n);
                                    ...
                                    Element maximum_bandwidth_el = (Element) maximum_bandwidth_nl
                                            .item(0);
                                    String s_maximum_bandwidth = getCharacterDataFromElement(maximum_bandwidth_el);

                                    float maximum_bandwidth = Float
                                            .parseFloat(s_maximum_bandwidth);
                            ...
                            /**
                             * NodeList SID_aux =
                             * element.getElementsByTagName("sid"); Element
                             * SID_e = (Element) SID_aux.item(0); if
                             * (SID_e!=null) { log.info("SID existe"); int SID =
                             * Integer
                             * .parseInt(getCharacterDataFromElement(SID_e));
                             * SIDS.put(router_id_addr,SID);
                             * log.info("SID of node: "+SID); } else {
                             * log.info("SID not found"); }
                             */

                            NodeList defaultmetric = element
                                    .getElementsByTagName("default_te_metric");
                            Element metric_aux = (Element) defaultmetric
                                    .item(0);
                            ...
                            NodeList unreserved_bandwidth_nl = element
                                    .getElementsByTagName("unreserved_bandwidth");
                            if (unreserved_bandwidth_nl != null) {
                                int num_u_b = unreserved_bandwidth_nl
                                        .getLength();
                                ...
                                if (num_u_b > 0) {
                                    ...
                                    (edge.getTE_info())
                                            .setUnreservedBandwidth(unreservedBandwidth);
                                    for (int k = 0; k < num_u_b; ++k) {
                                        Element unreserved_bandwidth_el = (Element) unreserved_bandwidth_nl
                                                .item(k);
                                        String s_unreserved_bandwidth = getCharacterDataFromElement(unreserved_bandwidth_el);

                                        String s_priority = unreserved_bandwidth_el
                                                .getAttributeNode("priority")
                                                .getValue();
                                        Integer priority = Integer
                                                .valueOf(s_priority);
                                        float unreserved_bandwidth = Float
                                                .parseFloat(s_unreserved_bandwidth);

                                        (unreservedBandwidth
                                                .getUnreservedBandwidth())[priority] = unreserved_bandwidth;
                                    }
                                }

                            }

                            NodeList maximum_wlans_nl = element
                                    .getElementsByTagName("number_wlans");
                            if (maximum_wlans_nl != null) {
                                if (maximum_wlans_nl.getLength() > 0) {
                                    ...
                                    Element number_wlan_el = (Element) maximum_wlans_nl
                                            .item(0);
                                    String s_number_wlans = getCharacterDataFromElement(number_wlan_el);

                                    int number_wlans = Integer
                                            .parseInt(s_number_wlans.replace(
                                                    "\n", "").replaceAll("\\s",
                                                    ""));
                                    (edge.getTE_info())
                                            .setNumberWLANs(number_wlans);
                            ...
                            if (commonBitmapLabelSet) {
                                if (lambdaEnd != Integer.MAX_VALUE)
                                    edge.getTE_info().createBitmapLabelSet(
                                            numLabels, grid, cs, n, lambdaIni,
                                            lambdaEnd);
                                else
                                    edge.getTE_info().createBitmapLabelSet(
                                            numLabels, grid, cs, n);
                            ...
                            NodeList availableLabels_node = element
                                    .getElementsByTagName("AvailableLabels");
                            if (availableLabels_node != null) {
                                for (int k = 0; k < availableLabels_node
                                        .getLength(); k++) {
                                    Element availableLabels_e = (Element) availableLabels_node
                                            .item(k);
                                    NodeList labelSetField_node = availableLabels_e
                                    ...
                                    if (labelSetField_el
                                        ...
                                        NodeList bitmap_node = labelSetField_el
                                                .getElementsByTagName("bitmap");
                                        int result = 0;
                                        Element bitmap_e = (Element) bitmap_node
                                                .item(0);
                                        if (bitmap_e != null) {
                                            String bitmap_string = getCharacterDataFromElement(bitmap_e);
                                            log.debug("Bitmap read: "
                                                    + bitmap_string);
                                            for (int p = 0; p < bitmap_string
                                                    .length(); p++)
                                            ...
                                            ((BitmapLabelSet) edge.getTE_info()
                                                    .getAvailableLabels()
                                                    .getLabelSet())
                                                    .setBytesBitmap(bitmap);
                                        }
                                        float min_frequency;

                                        grid = Integer.parseInt(baseLabel_e
                                                .getAttributeNode("grid")
                                                .getValue());
                            ...
                            try {
                                if (graph.containsEdge(s_router_id_addr,
                                        d_router_id_addr)) {
                                    graph.getEdge(s_router_id_addr,
                                            d_router_id_addr).setNumberFibers(
                                            graph.getEdge(s_router_id_addr,
                                                    d_router_id_addr)
                                                    .getNumberFibers() + 1);
                                } else {
                                    log.debug("s_router_id_addr: "
                                            + s_router_id_addr.toString()
                                            + "; d_router_id_addr: "
                                            + d_router_id_addr.toString()
                                            + "; edge: " + edge);
                                    graph.addEdge(s_router_id_addr,
                                            d_router_id_addr, edge);
                                    graph.getEdge(s_router_id_addr,
                                            d_router_id_addr)
                                            .setNumberFibers(1);
                                }
                                log.info("Adding edge from "
                                        + s_router_id_addr.toString() + " to "
                                        + d_router_id_addr.toString() + "; \n "
                                        + edge);

                            } catch (Exception e) {
                                log.error("Problem with source "
                                        + s_router_id_addr + " destination "
                                        + d_router_id_addr);
                                e.printStackTrace();
                                System.exit(-1);
                            }
    ...
    /*
     * private static SimpleDirectedWeightedGraph<Object, IntraDomainEdge>
     * readGraphIntraDomain(Element element1, String layer, Logger log){
     * 
     * Object router_id_addr = null;
     * 
     * HashMap<Inet4Address, Integer >SIDS = new
     * HashMap<Inet4Address,Integer>(); HashMap<DataPathID, Integer >SIDSDP =
     * new HashMap<DataPathID,Integer>();
     * 
     * SimpleDirectedWeightedGraph<Object, IntraDomainEdge> graph = new
     * SimpleDirectedWeightedGraph<Object,
     * IntraDomainEdge>(IntraDomainEdge.class); //NodeList nodes_domains =
     * doc.getElementsByTagName("domain"); boolean readNetwork=false; //Element
     * element1 = (Element) nodes_domains.item(j); String domain_id = "";
     * 
     * if (layer!=null){ NodeList domain_layer =
     * element1.getElementsByTagName("layer"); if (domain_layer.getLength()==1){
     * Element layer_type = (Element) domain_layer.item(0); log.info("Layer: " +
     * layer_type.getAttributeNode("type").getValue());
     * log.info("Reading network topology"); if
     * (layer_type.getAttributeNode("type").getValue().equals(layer)){
     * readNetwork = true;
     * 
     * } if (layer.equals("interlayer")){ readNetwork = true; } } }else {
     * readNetwork=true; } if (readNetwork){ Element element_domain = element1;
     * NodeList nodes_domain_id =
     * element_domain.getElementsByTagName("domain_id"); for (int k = 0; k <
     * nodes_domain_id.getLength(); k++) { Element domain_id_e = (Element)
     * nodes_domain_id.item(0); domain_id =
     * getCharacterDataFromElement(domain_id_e);
     * //log.info("Looking for nodes in domain: " + domain_id); }
     * 
     * NodeList nodes = element1.getElementsByTagName("node"); for (int i = 0; i
     * < nodes.getLength(); i++) { Element element = (Element) nodes.item(i);
     * NodeList router_id_node = element.getElementsByTagName("router_id");
     * Element router_id_e = (Element) router_id_node.item(0); String router_id
     * = getCharacterDataFromElement(router_id_e);
     * 
     * //log.info("Adding router_id " + router_id); router_id_addr =
     * EdgeUtils.getEdge(router_id); graph.addVertex(router_id_addr);
     * 
     * //log.info("About to look for SID"); NodeList SID_aux =
     * element.getElementsByTagName("sid"); Element SID_e = (Element)
     * SID_aux.item(0); if (SID_e!=null) { //log.info("SID existe"); int SID =
     * Integer.parseInt(getCharacterDataFromElement(SID_e));
     * 
     * try { //Router_type: DatapathID SIDS.put((Inet4Address)
     * router_id_addr,SID); //log.info("SID of node Inet4Address: "+SID); }
     * catch (Exception e) { //Router_type: DatapathID
     * SIDSDP.put((DataPathID)router_id_addr, SID);
     * //log.info("SID of node DataPathID: "+SID); } } else {
     * //log.info("SID not found"); }
     * 
     * } } return graph; }
     */

    public static Hashtable<String, TEDB> readMultipleDomainSimpleNetworks(
            String fileName, String layer, boolean allDomains, int lambdaIni,
            int lambdaEnd, boolean isSSONnetwork) {
        Logger log = LoggerFactory.getLogger("BGP4Peer");
        Object router_id_addr = null;
        ...
        try {
            ...
            for (int j = 0; j < nodes_domains.getLength(); j++) {
                ...
                if (readNetwork) {
                    ...
                    NodeList itResourcesElement = element1
                            .getElementsByTagName("it_resources");
                    for (int i = 0; i < itResourcesElement.getLength(); i++) {
                        Element element = (Element) itResourcesElement.item(i);

                        NodeList itResourcesControllerITList = element
                                .getElementsByTagName("controller_it");
                        Element itResourcesControllerITElement = (Element) itResourcesControllerITList
                                .item(0);
                        String itResourcesControllerIT = getCharacterDataFromElement(itResourcesControllerITElement);

                        NodeList itResourcesCpuList = element
                                .getElementsByTagName("cpu");
                        Element itResourcesCpuElement = (Element) itResourcesCpuList
                                .item(0);
                        String itResourcesCpu = getCharacterDataFromElement(itResourcesCpuElement);

                        NodeList itResourcesMemList = element
                                .getElementsByTagName("mem");
                        Element itResourcesMemElement = (Element) itResourcesMemList
                                .item(0);
                        String itResourcesMem = getCharacterDataFromElement(itResourcesMemElement);

                        NodeList itResourcesStorageList = element
                                .getElementsByTagName("storage");
                        Element itResourcesStorageElement = (Element) itResourcesStorageList
                                .item(0);
                        ...
                        if (itResourcesControllerIT != null)
                            itResources
                                    .setControllerIT(itResourcesControllerIT);
                ...
                tedb.setDomainID((Inet4Address) Inet4Address
                        .getByName(domain_id));
            ...
            for (int j = 0; j < nodes_domains.getLength(); j++) {
                ...
                if (readNetwork) {
                    ...
                    for (int i = 0; i < edgeCommon.getLength(); i++) {
                        Element edgeCommonElement = (Element) edgeCommon
                                .item(i);
                        tE_info_common = readTE_INFOfromXml(tE_info_common,
                                edgeCommonElement, false, numLabels, grid, cs,
                    ...
                    for (int i = 0; i < edges.getLength(); i++) {
                        ...
                        if (attr_type == null) {
                            ...
                            type = attr_type.getValue();
                            /*
                             * if (allDomains){ if (type.equals("interdomain")){
                             * type="intradomain"; } }
                             */
                            // else if (type.equals("interlayer")){
                        ...
                        if (type.equals("intradomain")) {
                            ...
                            // TE Link information
                            edge.setTE_info(readTE_INFOfromXml(tE_info_common,
                                    element, commonBitmapLabelSet, numLabels,
                                    grid, cs, n, lambdaIni, lambdaEnd));
                            ...
                            if (availableLabels_node != null) {
                                for (int k = 0; k < availableLabels_node
                                    ...
                                    if (labelSetField_el
                                        ...
                                        if (bitmap_e != null) {
                                            String bitmap_string = getCharacterDataFromElement(bitmap_e);
                                            System.out.println("Bitmap read: "
                                                    + bitmap_string);
                                            for (int p = 0; p < bitmap_string
                                                    .length(); p++)
                                                result = (int) (result + Math
                        ...
                        if (type.equals("interdomain")) {
                            ...
                            TE_Information tE_info = readTE_INFOfromXml(
                                    tE_info_common, element, false, numLabels,
                                    grid, cs, n, 0, Integer.MAX_VALUE);
                            edge.setTE_info(tE_info);
                            NodeList source = element
                                    .getElementsByTagName("source");
                            Element source_router_el = (Element) source.item(0);
                            ...
                            try { // s_router_id_addr type: Inet4Address
                                s_router_id_addr = (Inet4Address) Inet4Address
                                        .getByName(s_r_id);
                            } catch (Exception e) {// s_router_id_addr type:
                                                   // DataPathID
                                s_router_id_addr = DataPathID.getByName(s_r_id);
                            }
                            Inet4Address source_domain_id = (Inet4Address) Inet4Address
                                    .getByName(domain_id);
                            log.debug("Edge Source domain_id: "
                                    + source_domain_id);
                            ...
                            try { // d_router_id_addr type: Inet4Address
                                d_router_id_addr = (Inet4Address) Inet4Address
                                        .getByName(d_r_id);
                            } catch (Exception e) { // d_router_id_addr type:
                                                    // DataPathID
                                d_router_id_addr = DataPathID.getByName(d_r_id);
                            }
                            // Inet4Address dest_domain_id =
                            // router_id_domain_ed.get(d_router_id_addr);
                            ...
                            if (domainTEDB.getInterDomainLinks() == null) {
                                ...
                                domainTEDB
                                        .setInterDomainLinks(interDomainLinks);
                                log.info("New interdomain link: "
                                        + s_router_id_addr.toString() + " --> "
                                        + d_router_id_addr.toString());// +"; \n "+edge);
    ...
    public static DirectedWeightedMultigraph<Object, InterDomainEdge> readMDNetwork(
            VirtualizerSchema objTopology, String LearntFrom) {
        Logger log = LoggerFactory.getLogger("BGP4Peer");

        DirectedWeightedMultigraph<Object, InterDomainEdge> graph;
        if (TopologyServerUnify.getActualTed().getMdTed()
                .getInterDomainLinks2() != null) {
            graph = TopologyServerUnify.getActualTed().getMdTed()
                    .getInterDomainLinks2();
        } else
            graph = new DirectedWeightedMultigraph<Object, InterDomainEdge>(
                    InterDomainEdge.class);

        // MultiDomainTEDB ted =
        // TopologyServerUnify.getActualTed().getMdTed().ge;
        // LinkedList<InterDomainEdge> graph1 = ted.getInterDomainLinks();
        // graph1.get(0).

        // DirectedWeightedMultigraph<Object, InterDomainEdge> graph = new
        ...
        try {
            ...
            Nodes nodes = mapper.convertValue(objTopology.getNodes(),
                    Nodes.class);
            ...
            for (Link objInterEdge : mapper.convertValue(
                ...
                InterDomainEdge edge = new InterDomainEdge();
                TE_Information tE_info = new TE_Information();
                if (objInterEdge.getResources() != null
                        && objInterEdge.getResources().getDelay() != null) {

                    int undirDelayLinkValue = Integer.parseInt(objInterEdge
                            .getResources().getDelay());
                ...
                if (!source_domain_id.equals(dest_domain_id)) {
                    log.info("Adding interdomain link "
                            + source_domain_id.toString() + "-->"
                            + dest_domain_id.toString() + " learnt "
                            + edge.getLearntFrom());// edge.toString());
                    // Only add if the source and destination domains are
                    // different
    ...
    public static Hashtable<String, TEDB> readMultipleDomainSimpleNetworks(
            VirtualizerSchema objTopology, String layer, boolean allDomains,
            int lambdaIni, int lambdaEnd, boolean isSSONnetwork,
            String learntFrom) {
        Logger log = LoggerFactory.getLogger("BGP4Peer");
        Object router_id_addr = null;
        Object s_router_id_addr = null;
        ...
        Hashtable<String, TEDB> TEDBs;

        TopologiesDataBase ted = TopologyServerUnify.getActualTed();
        if (ted != null)
            TEDBs = ted.getTeds();
        else
            TEDBs = new Hashtable<String, TEDB>();
         ...
         * System.out.println("Serving topology with id: " + entry.getKey()); if
         * (entry.getValue() instanceof DomainTEDB) { SimpleTEDB x =
         * (SimpleTEDB) entry.getValue(); System.out.println(x);
         * 
         * } else if (entry.getValue() instanceof MultiDomainTEDB) { //
         * Interdomain links MultiDomainTEDB y = (MultiDomainTEDB)
         * entry.getValue(); System.out.println(y);
         * 
        ...
        try {
            ...
            for (Node objNode : nodes.getNode()) {
                domain_id = objNode.getId();
                SimpleTEDB sTEDB = (SimpleTEDB) ted.getTeds().get(domain_id);
                SimpleTEDB tedb;
                if (sTEDB != null)
                    tedb = sTEDB;
                else
                ...
                if (tedb.getNetworkGraph() != null)
                    graph = tedb.getNetworkGraph();
                else
                    graph = new SimpleDirectedWeightedGraph<Object, IntraDomainEdge>(
                            IntraDomainEdge.class);
                ...
                if (tedb.getItResources() != null)
                    itResources = tedb.getItResources();
                else
                    itResources = new IT_Resources();

                for (MetadataMetadata met : objNode.getMetadata()) {
                    if (met.getKey().equals("unify-slor")) {
                        itResources.setControllerIT(met.getValue());
                        itResources.setLearntFrom(learntFrom);
                    }
                }
                SoftwareResource swResource = mapper.convertValue(
                        objNode.getResources(), SoftwareResource.class);
                if (swResource != null) {
                    if (!swResource.getCpu().isEmpty())
                        itResources.setCpu(swResource.getCpu());
                    if (!swResource.getMem().isEmpty())
                        itResources.setMem(swResource.getMem());
                    if (!swResource.getStorage().isEmpty())
                        itResources.setStorage(swResource.getStorage());
                    log.info("set learn from for it resources "
                            + itResources.toString());
                }
                tedb.setItResources(itResources);

                ReachabilityEntry reachabilityEntry;
                if (tedb.getReachabilityEntry() != null)
                    reachabilityEntry = tedb.getReachabilityEntry();
                else
                    reachabilityEntry = new ReachabilityEntry();
                String ipv4_address_str = null;
                String prefix_str = null;
                for (MetadataMetadata met : objNode.getMetadata()) {
                    if (met.getKey().equals("reachability_ipv4_address"))
                        ipv4_address_str = met.getValue();
                    if (met.getKey().equals("reachability_prefix"))
                        prefix_str = met.getValue();
                }
                if (ipv4_address_str != null && prefix_str != null) {
                    ...
                    tedb.setReachabilityEntry(reachabilityEntry);
                }
            ...
            for (Node objEdge : nodes.getNode()) {

                SimpleDirectedWeightedGraph<Object, IntraDomainEdge> graph = null;

                domain_id = objEdge.getId();
                log.debug("Looking for links in domain: " + domain_id);

                SimpleTEDB domainTEDB = (SimpleTEDB) TEDBs.get(domain_id);
                graph = domainTEDB.getNetworkGraph();

                LinksSchema links = mapper.convertValue(objEdge.getLinks(),
                        LinksSchema.class);
                for (Link objLink : links.getLink()) {
                    log.debug("New Link");
                    String type = "intradomain";
                    log.debug("type::" + type);

                    IntraDomainEdge edge = new IntraDomainEdge();
                    edge.setLearntFrom(learntFrom);

                    s_router_id_addr = EdgeUtils.getEdge(objLink.getSrc()
                            .split("\\[")[2].split("\\]")[0].split("=")[1]);

                    d_router_id_addr = EdgeUtils.getEdge(objLink.getDst()
                            .split("\\[")[2].split("\\]")[0].split("=")[1]);
                    ...
                    if (SIDS.get(s_router_id_addr) != null
                        ...
                        log.debug("edge.getDst_sid(): " + edge.getDst_sid());
                            ...
                            && SIDSDP.get(d_router_id_addr) != null) {
                        log.debug("setting SIDSDP src: "
                                + SIDSDP.get(s_router_id_addr) + " dst: "
                                + SIDSDP.get(d_router_id_addr));
                        edge.setSrc_sid(SIDSDP.get(s_router_id_addr));
                        edge.setDst_sid(SIDSDP.get(d_router_id_addr));
                        log.debug("edge.getSrc_sid(): " + edge.getSrc_sid());
                    ...
                    TE_Information tE_info = new TE_Information();
                    if (objLink.getResources().getDelay() != null) {

                        int undirDelayLinkValue = Integer.parseInt(objLink
                                .getResources().getDelay());
                        UndirectionalLinkDelayDescriptorSubTLV uldSTLV = new UndirectionalLinkDelayDescriptorSubTLV();
                        uldSTLV.setDelay(undirDelayLinkValue);
                        tE_info.setUndirLinkDelay(uldSTLV);
                    ...
                    try {
                        if (graph.containsEdge(s_router_id_addr,
                                d_router_id_addr)) {
                            log.debug("New Intradomain Edge");
                            graph.getEdge(s_router_id_addr, d_router_id_addr)
                                    .setNumberFibers(
                                            graph.getEdge(s_router_id_addr,
                                                    d_router_id_addr)
                                                    .getNumberFibers() + 1);
                            log.debug("s_router_id_addr: "
                                    + s_router_id_addr.toString()
                                    + "; d_router_id_addr: "
                                    + d_router_id_addr.toString() + "; edge: "
                                    + edge);
                            log.info("New intradomain link: "
                                    + s_router_id_addr.toString() + " --> "
                                    + d_router_id_addr.toString());// +" \n "+edge);
                            ...
                            if (graph.containsVertex(d_router_id_addr) == false) {
                                ...
                                graph.addEdge(s_router_id_addr,
                                        d_router_id_addr, edge);
                                graph.getEdge(s_router_id_addr,
                                        d_router_id_addr).setNumberFibers(1);
                                log.debug("New Intradomain Edge");
                                log.info("New intradomain link: "
                                        + s_router_id_addr.toString() + " --> "
                                        + d_router_id_addr.toString()
                                        + " learnt " + edge.getLearntFrom());// +" \n "+edge);

                            }
                        }

                    } catch (Exception e) {
                        log.error("Problem with source " + s_router_id_addr
                                + " destination " + d_router_id_addr);
                        e.printStackTrace();
                        System.exit(-1);
                    }
                }
    ...
    public static Hashtable<String, TEDB> readMultipleDomainSimpleNetworks(
            String fileName, String layer, boolean allDomains, int lambdaIni,
            int lambdaEnd, boolean isSSONnetwork, String learntFrom) {
        Logger log = LoggerFactory.getLogger("BGP4Peer");
        Object router_id_addr = null;
        Object s_router_id_addr = null;
        Object d_router_id_addr = null;
        ...
        try {
            ...
            for (int j = 0; j < nodes_domains.getLength(); j++) {
                ...
                if (readNetwork) {
                    ...
                    /********** dlachosper **************************************************/
                    NodeList itReachabilityElement = element1
                            .getElementsByTagName("reachability_entry");
                    for (int i = 0; i < itReachabilityElement.getLength(); i++) {

                        ReachabilityEntry reachabilityEntry = new ReachabilityEntry();
                        FileTEDBUpdater.getDomainReachabilityFromDomainID(
                                element1, reachabilityEntry);
                        tedb.setReachabilityEntry(reachabilityEntry);
                    }
                    /********** dlachosper **************************************************/
    ...
    private static TE_Information readTE_INFOfromXml(
            TE_Information tE_info_common, Element element,
            Boolean commonBitmapLabelSet, int numLabels, int grid, int cs,
            int n, int lambdaIni, int lambdaEnd) {

        TE_Information tE_info = new TE_Information();
        if (commonBitmapLabelSet) {
            if (lambdaEnd != Integer.MAX_VALUE)
                tE_info.createBitmapLabelSet(numLabels, grid, cs, n, lambdaIni,
                        lambdaEnd);
            else
        ...
        } else if (tE_info_common != null
                && tE_info_common.getMaximumBandwidth() != null) {
            MaximumBandwidth maximumBandwidth = new MaximumBandwidth();
            maximumBandwidth.setMaximumBandwidth(tE_info_common
                    .getMaximumBandwidth().getMaximumBandwidth());
            tE_info.setMaximumBandwidth(maximumBandwidth);
        }
        /**
         * NodeList SID_aux = element.getElementsByTagName("sid"); Element SID_e
         * = (Element) SID_aux.item(0); if (SID_e!=null) {
         * log.info("SID existe"); int SID =
         * Integer.parseInt(getCharacterDataFromElement(SID_e));
         * SIDS.put(router_id_addr,SID); log.info("SID of node: "+SID); } else {
         * log.info("SID not found"); }
         */
        ...
        if (metric_aux != null) {
            ...
            tE_info.setDefaultTEMetric(defaultTeMetric);
                ...
                && tE_info_common.getDefaultTEMetric() != null) {
            DefaultTEMetricLinkAttribTLV defaultTeMetric = new DefaultTEMetricLinkAttribTLV();
            defaultTeMetric.setLinkMetric(tE_info_common.getDefaultTEMetric()
                    .getLinkMetric());
            tE_info.setDefaultTEMetric(defaultTeMetric);
        }
        ...
        if (unreserved_bandwidth_nl != null) {
            ...
            if (num_u_b > 0) {
                ...
                for (int k = 0; k < num_u_b; ++k) {
                    ...
                    String s_priority = unreserved_bandwidth_el
                            .getAttributeNode("priority").getValue();
        ...
        NodeList undirDelayLinks = element
                .getElementsByTagName("undir_delay_link");
        ...
        if (undirDelayLink != null) {
            ...
            tE_info.setUndirLinkDelay(uldSTLV);
                ...
                && tE_info_common.getUndirLinkDelay() != null) {
            UndirectionalLinkDelayDescriptorSubTLV uldSTLV = new UndirectionalLinkDelayDescriptorSubTLV();
            uldSTLV.setDelay(tE_info_common.getUndirLinkDelay().getDelay());
            tE_info.setUndirLinkDelay(uldSTLV);
        }

        NodeList undirDelayVars = element
                .getElementsByTagName("undir_delay_variation");
        ...
        if (undirDelayVar != null) {
            ...
            tE_info.setUndirDelayVar(udvSTLV);
                ...
                && tE_info_common.getUndirDelayVar() != null) {
        ...
        NodeList undirLinkLosss = element
                .getElementsByTagName("undir_link_loss");
        ...
        if (undirLinkLoss != null) {
            ...
            tE_info.setUndirLinkLoss(uSTLV);
                ...
                && tE_info_common.getUndirLinkLoss() != null) {
        ...
        NodeList undirReBws = element
                .getElementsByTagName("undir_residual_bandwidth");
        ...
        if (undirReBw != null) {
            ...
            tE_info.setUndirResidualBw(uSTLV);
                ...
                && tE_info_common.getUndirResidualBw() != null) {
            UndirectionalResidualBandwidthDescriptorSubTLV uldSTLV = new UndirectionalResidualBandwidthDescriptorSubTLV();
            uldSTLV.setResidualBw(tE_info_common.getUndirResidualBw()
                    .getResidualBw());
            tE_info.setUndirResidualBw(uldSTLV);
        }

        NodeList undirAvalBws = element
                .getElementsByTagName("undir_available_bandwidth");
        ...
        if (undirAvalBw != null) {
            ...
            tE_info.setUndirAvailableBw(uSTLV);
                ...
                && tE_info_common.getUndirAvailableBw() != null) {
            UndirectionalAvailableBandwidthDescriptorSubTLV uldSTLV = new UndirectionalAvailableBandwidthDescriptorSubTLV();
            uldSTLV.setAvailableBw(tE_info_common.getUndirAvailableBw()
                    .getAvailableBw());
            tE_info.setUndirAvailableBw(uldSTLV);
        }

        NodeList undirUtilBws = element
                .getElementsByTagName("undir_utilized_bandwidth");
        ...
        if (undirUtilBw != null) {
            ...
            tE_info.setUndirUtilizedBw(uSTLV);
                ...
                && tE_info_common.getUndirUtilizedBw() != null) {
            UndirectionalUtilizedBandwidthDescriptorSubTLV uldSTLV = new UndirectionalUtilizedBandwidthDescriptorSubTLV();
            uldSTLV.setUtilizedBw(tE_info_common.getUndirUtilizedBw()
                    .getUtilizedBw());
            tE_info.setUndirUtilizedBw(uldSTLV);
        }

        NodeList minMaxDelays = element
                .getElementsByTagName("undir_min_max_delay");
        ...
        if (minMaxDelay != null) {
            ...
            if (minDelay != null && maxDelay != null) {
                ...
                && tE_info_common.getMinMaxUndirLinkDelay() != null) {
            MinMaxUndirectionalLinkDelayDescriptorSubTLV uldSTLV = new MinMaxUndirectionalLinkDelayDescriptorSubTLV();
            uldSTLV.setHighDelay(tE_info_common.getMinMaxUndirLinkDelay()
                    .getHighDelay());
            uldSTLV.setLowDelay(tE_info_common.getMinMaxUndirLinkDelay()
                    .getLowDelay());
    ...
    public static SimpleDirectedWeightedGraph<Object, IntraDomainEdge> readITNetwork(
            String fileName) {
        Logger log = LoggerFactory.getLogger("BGP4Peer");
        SimpleDirectedWeightedGraph<Object, IntraDomainEdge> graph = new SimpleDirectedWeightedGraph<Object, IntraDomainEdge>(
                IntraDomainEdge.class);
            ...
            for (int j = 0; j < nodes_domains.getLength(); j++) {
                ...
                for (int i = 0; i < nodes.getLength(); i++) {
                    ...
                    if (SID_e != null) {
                        ...
                        try { // router_id_addr type: Inet4Address
                            SIDS.put((Inet4Address) router_id_addr, SID);
                        } catch (Exception e) { // router_id_addr type:
                                                // DataPathID
                            SIDSDP.put((DataPathID) router_id_addr, SID);
                        }
                    }
                ...
                for (int i = 0; i < it_sites.getLength(); i++) {
                    ...
                    NodeList it_site_id_node = element
                    ...
                    try { // it_site_id_addr type: Inet4Address
                        it_site_id_addr = (Inet4Address) Inet4Address
                                .getByName(it_site_id);
                ...
                for (int i = 0; i < storages.getLength(); i++) {
                    ...
                    NodeList resource_id_node = element
                            .getElementsByTagName("resource_id");
                    ...
                    try {
                        resource_addr = (Inet4Address) Inet4Address
                                .getByName(resource_id);
                    } catch (Exception e) {
                ...
                for (int i = 0; i < edges.getLength(); i++) {
                    ...
                    // We only want those routers which have type="intradomain"
                    // //MARTA
                    if (element.getAttributeNode("type").getValue()
                            .equals("intradomain")) {// MARTA
                        // IntraDomainEdge edge = new IntraDomainEdge();
                        ...
                        NodeList source_it_site_id = source_router_el
                                .getElementsByTagName("it_site_id");
                        if (source_it_site_id.getLength() > 0) {
                            Element source_it_site_id_el = (Element) source_it_site_id
                                    .item(0);
                            ...
                            try { // s_it_site_id_addr type: Inet4Address
                                s_it_site_id_addr = (Inet4Address) Inet4Address
                                        .getByName(s_itsite_id);
                            } catch (Exception e) { // s_it_site_id_addr type:
                                                    // DataPathID
                                s_it_site_id_addr = DataPathID
                                        .getByName(s_itsite_id);
                            }
                            s_id_addr = s_it_site_id_addr;
                        }

                        NodeList source_resource_id = source_router_el
                                .getElementsByTagName("resource_id");
                        if (source_resource_id.getLength() > 0) {
                            Element source_resource_id_el = (Element) source_resource_id
                                    .item(0);
                            String s_resource_id = getCharacterDataFromElement(source_resource_id_el);
                            log.debug("Edge Source resource_id: "
                                    + s_resource_id);

                            try {// s_resource_id_addr type: Inet4Address
                                s_resource_id_addr = (Inet4Address) Inet4Address
                                        .getByName(s_resource_id);
                            } catch (Exception e) { // s_resource_id_addr type:
                                                    // DataPathID
                                s_resource_id_addr = DataPathID
                                        .getByName(s_resource_id);
                            }
                        ...
                        NodeList dest_it_site_id_nl = dest_el
                                .getElementsByTagName("it_site_id");
                        if (dest_it_site_id_nl.getLength() > 0) {
                            Element dest_it_site_id_el = (Element) dest_it_site_id_nl
                                    .item(0);
                            String d_it_site_id = getCharacterDataFromElement(dest_it_site_id_el);
                            log.debug("Edge Destination IT_site_id: "
                                    + d_it_site_id);

                            try { // d_it_site_id_addr type: Inet4Address
                                d_it_site_id_addr = (Inet4Address) Inet4Address
                                        .getByName(d_it_site_id);
                            } catch (Exception e) { // d_it_site_id_addr type:
                                                    // DataPathID
                                d_it_site_id_addr = DataPathID
                                        .getByName(d_it_site_id);
                            }
                            d_id_addr = d_it_site_id_addr;
                        }

                        NodeList dest_resource_id_nl = dest_el
                                .getElementsByTagName("resource_id");
                        if (dest_resource_id_nl.getLength() > 0) {
                            Element dest_resource_id_el = (Element) dest_resource_id_nl
                                    .item(0);
                            String d_resource_id = getCharacterDataFromElement(dest_resource_id_el);
                            log.debug("Edge Destination resource_id: "
                                    + d_resource_id);
                            try { // d_resource_id_addr type: Inet4Address
                                d_resource_id_addr = (Inet4Address) Inet4Address
                                        .getByName(d_resource_id);
                            } catch (Exception e) { // d_resource_id_addr type:
                                                    // DataPathID
                                d_resource_id_addr = DataPathID
                                        .getByName(d_resource_id);
                        ...
                        // Anyadimos los SID
                        if (SIDS.get(s_id_addr) != null
                                && SIDS.get(d_id_addr) != null) {
    ...
    /**
     * Reads the inter-domain Topology from a topology XML file. Needs update to
     * work with datapath IDs.
     * 
     * @param fileName
     *            Name of the XML
     * @return Graph of the network.
        ...
        try {
            ...
            for (int i = 0; i < edgeCommon.getLength(); i++) {
                ...
                for (int k = 0; k < availableLabels_node.getLength(); k++) {
                    ...
                    Element labelSetField_el = (Element) labelSetField_node
                            .item(0);
                    if (labelSetField_el.getAttributeNode("type").getValue()
                            .equals("4")) {// Tengo BitMapSet
                        // Crear un BitMapSet
                        ...
                        grid = Integer.parseInt(baseLabel_e.getAttributeNode(
                                "grid").getValue());
                        ...
                        boolean n_frequency_included = false;
                        if (baseLabel_e.getAttributeNode("n") != null) {
                            n = Integer.parseInt(baseLabel_e.getAttributeNode(
                                    "n").getValue());
                            n_frequency_included = true;
                        } else if (baseLabel_e
                                .getAttributeNode("min_frequency") != null) {
            ...
            for (int i = 0; i < edges.getLength(); i++) {
                ...
                Element element = (Element) edges.item(i);
                InterDomainEdge edge = new InterDomainEdge();
                TE_Information tE_info = readTE_INFOfromXml(null, element,
                        false, numLabels, grid, cs, n, 0, Integer.MAX_VALUE);
                edge.setTE_info(tE_info);
                ...
                if (!source_domain_id.equals(dest_domain_id)) {
                    log.info("Adding interdomain link "
                            + source_domain_id.toString() + "-->"
                            + dest_domain_id.toString());// edge.toString());
                    // Only add if the source and destination domains are
                    // different
                    graph.addEdge(source_domain_id, dest_domain_id, edge);
                }
    ...
    public static DirectedWeightedMultigraph<Object, InterDomainEdge> readITMDNetwork(
            String fileName) {
        Logger log = LoggerFactory.getLogger("BGP4Peer");
        DirectedWeightedMultigraph<Object, InterDomainEdge> graph = new DirectedWeightedMultigraph<Object, InterDomainEdge>(
                InterDomainEdge.class);
        ...
        try {
            ...
            for (int j = 0; j < nodes_domains.getLength(); j++) {
                ...
                for (int i = 0; i < nodes.getLength(); i++) {
                    ...
                    NodeList domain_id_node = element
                            .getElementsByTagName("domain_id");
                    router_id_domain_ed.put(router_id_addr, domain_id);
                    // graph.addVertex(router_id_addr);
                ...
                NodeList ITsites = element_domain
                        .getElementsByTagName("it_site");
                for (int i = 0; i < ITsites.getLength(); i++) {
                    ...
                    Inet4Address it_site_id_addr = (Inet4Address) Inet4Address
                            .getByName(it_site_id);
                ...
                NodeList storages = element_domain
                        .getElementsByTagName("storage");
                for (int i = 0; i < storages.getLength(); i++) {
                    Element element = (Element) storages.item(i);
                    NodeList resource_id_node = element
                    ...
                    Inet4Address resource_id_addr = (Inet4Address) Inet4Address
                            .getByName(resource_id);
            ...
            for (int i = 0; i < edges.getLength(); i++) {
                ...
                if (source_router_id.getLength() > 0) {
                    ...
                    s_router_id_addr = (Inet4Address) Inet4Address
                            .getByName(s_r_id);
                    source_domain_id = (Inet4Address) router_id_domain_ed
                            .get(s_router_id_addr);
                ...
                if (source_it_site_id.getLength() > 0) {
                    ...
                    s_itsite_id_addr = (Inet4Address) Inet4Address
                            .getByName(s_itsite_id);
                    source_domain_id = (Inet4Address) it_site_id_domain_ed2
                            .get(s_itsite_id_addr);
                    log.info("Edge Source ITsite domain_id: "
                            + source_domain_id);
                }
                ...
                if (source_resource_id.getLength() > 0) {
                    ...
                    s_resource_id_addr = (Inet4Address) Inet4Address
                            .getByName(s_resource_id);
                    source_domain_id = (Inet4Address) resource_id_domain_ed
                            .get(s_resource_id_addr);
                    log.info("Edge Source ITsite domain_id: "
                            + source_domain_id);
                }
                ...
                if (dest_router_id_nl.getLength() > 0) {
                    ...
                    d_router_id_addr = (Inet4Address) Inet4Address
                            .getByName(d_r_id);
                    dest_domain_id = (Inet4Address) router_id_domain_ed
                            .get(d_router_id_addr);
                ...
                if (dest_it_site_id_nl.getLength() > 0) {
                    ...
                    d_itsite_id_addr = (Inet4Address) Inet4Address
                            .getByName(d_itsite_id);
                    dest_domain_id = (Inet4Address) it_site_id_domain_ed2
                            .get(d_itsite_id_addr);
                ...
                if (dest_resource_id_nl.getLength() > 0) {
                    ...
                    d_resource_id_addr = (Inet4Address) Inet4Address
                            .getByName(d_resource_id);
                    dest_domain_id = (Inet4Address) resource_id_domain_ed
                            .get(d_resource_id_addr);
                    log.info("Destination ITsite domain_id: " + dest_domain_id);
                }
            ...
            for (int i = 0; i < edgeCommon.getLength(); i++) {
                ...
                for (int k = 0; k < availableLabels_node.getLength(); k++) {
                    ...
                    if (labelSetField_el.getAttributeNode("type").getValue()
                        ...
                        if (n_frequency_included) {
                            createBitmapLabelSet(commonAvailableLabels,
                                    numLabels, grid, cs, n);
                        } else {
                            log.error("ERROR reading the xml file of the topology, you should enter <baseLabel grid=\"1\" cs=\"2\" n=\"-11\"></baseLabel> ");
                        }
    ...
    public static void createBitmapLabelSet(AvailableLabels availableLabels,
            int numLabels, int grid, int cs, int n) {
    ...
    public static void getDomainReachabilityFromFile(String fileName,
            ReachabilityEntry reachabilityEntry) {
        FileTEDBUpdater.getDomainReachabilityFromFile(fileName,
                reachabilityEntry, null);
    }
    ...
    public static void getDomainReachabilityFromDomainID(
            Element element_domain, ReachabilityEntry reachabilityEntry) {
        Logger log = LoggerFactory.getLogger("BGP4Peer");
        ...
        try {
            ...
            for (int i = 0; i < nodes.getLength(); i++) {
                ...
                Inet4Address ipv4_address = (Inet4Address) Inet4Address
                        .getByName(ipv4_address_str);
                reachabilityEntry.setAggregatedIPRange(ipv4_address);
                // FIXME: dudas de esto. Si asignar la IP o copiarla byte a byte
                // con arraycopy
                /*
    ...
    public static void getDomainReachabilityFromFile(
            String fileName,
                ...
                if (readNetwork) {
                    ...
                    for (int i = 0; i < nodes.getLength(); i++) {
                        ...
                        Inet4Address ipv4_address = (Inet4Address) Inet4Address
                                .getByName(ipv4_address_str);
                        reachabilityEntry.setAggregatedIPRange(ipv4_address);
                        // FIXME: dudas de esto. Si asignar la IP o copiarla
                        // byte a byte con arraycopy
                        /*
                         * System.arraycopy(ipv4_address.getAddress(), 0,
                         * reachabilityEntry.getAggregatedIPRange(), 0, 4);
                         */
     ...
     */
    public static LinkedList<InterDomainEdge> readInterDomainLinks(
            String fileName) {
        ...
        try {
            ...
            if (nodes_domains != null) {
                if (nodes_domains.getLength() >= 1) {
                    for (int j = 0; j < nodes_domains.getLength(); j++) {
                        Element element_domain = (Element) nodes_domains
                                .item(j);
                        NodeList nodes_domain_id = element_domain
                                .getElementsByTagName("domain_id");
                        Element domain_id_e = (Element) nodes_domain_id.item(0);
                        String domain_id_str = getCharacterDataFromElement(domain_id_e);
                        domain_id = (Inet4Address) Inet4Address
                                .getByName(domain_id_str);
                    }
                }
            }
            ...
            for (int i = 0; i < edges.getLength(); i++) {
                ...
                // if
                // (((String)element.getAttributeNode("type").getValue()).equals("interdomain"))
                // {
                ...
                if (element.getAttributeNode("type") != null) {
                    ...
                    if (element.getAttributeNode("type").getValue()
                        ...
                        Inet4Address s_router_id_addr = (Inet4Address) Inet4Address
                                .getByName(s_r_id);
                        // Read the source router interface identifier of the
                        // connection
                        NodeList source_if_id_nl = source_router_el
                                .getElementsByTagName("if_id");
                        Element source_if_id_el = (Element) source_if_id_nl
                        ...
                        int src_if_id = Integer.parseInt(s_source_if_id);
                        NodeList src_domain_nl = source_router_el
                                .getElementsByTagName("domain_id");
                        Inet4Address s_router_domain = null;

                        if (src_domain_nl != null) {
                            if (src_domain_nl.getLength() >= 1) {
                                Element domain_id_el = (Element) src_domain_nl
                                        .item(0);
                                String s_r_domain = getCharacterDataFromElement(
                                        domain_id_el).trim();
                                s_router_domain = (Inet4Address) Inet4Address
                                        .getByName(s_r_domain);
                        ...
                        Inet4Address d_router_domain = (Inet4Address) Inet4Address
                                .getByName(d_r_domain);
                        ...
                        if (connection.getTE_info() == null) {
                            TE_Information tE_info = new TE_Information();
                            if (commonBitmapLabelSet) {
                                tE_info.createBitmapLabelSet(numLabels, grid,
                                        cs, n);
                            }
    ...
    public static int readWSONLambdas(String fileName) {
        ...
        try {
            ...
            for (int j = 0; j < nodes_domains.getLength(); j++) {
                Element element_domain = (Element) nodes_domains.item(j);
                NodeList nodes_wl = element_domain
                        .getElementsByTagName("num_wavelengths");
    ...
    public static Hashtable<Object, Object> getITSites(String fileName) {
        ...
        try {
            DocumentBuilder builder2 = DocumentBuilderFactory.newInstance()
                    .newDocumentBuilder();
    ...
    public static Hashtable<StorageTLV, Object> getStorageCharacteristics(
            String fileName) {
        ...
        try {
            ...
            for (int j = 0; j < nodes_domains.getLength(); j++) {
                ...
                for (int i = 0; i < storages.getLength(); i++) {
                    ...
                    Inet4Address virtual_TI_site = (Inet4Address) Inet4Address
                            .getByName((element.getAttributeNode("it_site")
                                    .getValue()).toString());
                    costsubtlv.setUsageUnit((element
                            .getAttributeNode("UsageUnit").getValue())
                            .getBytes());
                    costsubtlv.setUnitaryPrice((element
                            .getAttributeNode("UnitaryPrice").getValue())
                            .getBytes());
                    costlist.add(costsubtlv);
                    storagesizesubtlv.setTotalSize(Integer.parseInt(element
                            .getAttributeNode("TotalSize").getValue()));
                    storagesizesubtlv.setAvailableSize(Integer.parseInt(element
                            .getAttributeNode("AvailableSize").getValue()));
