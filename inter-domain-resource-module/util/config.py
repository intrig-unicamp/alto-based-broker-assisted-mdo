...
from misc import VERBOSE, unicode_to_str, quit_with_error
#from pox_extension import POXCoreRegisterMetaClass
...
class ESCAPEConfig(object):
  ...
  DEFAULT_CONFIG_FILE = "./alto-escape/escape-config.json"  # relative to project root
  ...
  def load_config (self, config=None):
    ...
    if config:
      ...
      print("Load explicitly given config file: %s" % config)
    else:
      # No config file has been given
      print("No additional configuration has been given!")
    try:
      if config:
        # Load file
